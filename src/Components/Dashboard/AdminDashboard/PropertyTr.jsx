...
const PropertyTr = ({ item, refetch }) => {
    ...
    return (
        <tr className="bg-sky-300 border-b  dark:border-gray-700 hover:bg-gray-50">
            ...
            <td className="px-2 py-2 text-gray-900 font-medium">
                <button onClick={()=> handlePropertyRejected(_id)}  className='border hover:bg-red-700 hover:text-white border-red-700 text-xs font-bold px-2 py-1 rounded md cursor-pointer text-red-700'>{status == "rejected" ? 'Rejected' : 'Reject'}</button>
            </td>
